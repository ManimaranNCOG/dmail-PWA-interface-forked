...
import chain from "../../asserts/loader-icon/chain.png";
...
const Alert = (params) => {
    return (
        <div>
            <Modal className="modal-alert-header" open={params.open} onOk={params.close} onCancel={params.close} footer={null} >
                <div className='send-alert-body-content'>
                    <div> {params.body} </div>
                </div>
                {params.redirect && <div class="modal-wrap-button">
                    <button onClick={() => { params.navigate(params.redirectPage) }} class="button-submit-modal">{params.btnValue || "Okay"}</button>
                </div>
